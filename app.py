"""
Auren Leads — Flask Backend
Run: python app.py  →  open http://localhost:5000
DO NOT open index.html directly as a file — it won't work.
"""
...
DEFAULTS = {
    ...
    "active_cities": [
        ...
        "Nagpur","Indore","Lucknow","Coimbatore","Bhopal","Gurgaon","Noida","Vadodara"
    ],
    "active_cats": [
...
def load_state():
    if os.path.exists(STATE_FILE):
        try:
            ...
            pass
    return {
        "automation_on": False, "last_run": None,
        "total_leads": 0, "running": False,
        "last_run_count": 0, "last_error": "",
...
def api_run_now():
    ...
    # save settings payload if sent along with run
    data = request.get_json(force=True, silent=True) or {}
    if data:
        save_settings(data)
...
def api_get_leads():
    try:
        ...
        if not s.get("sheet_id","").strip() or not s.get("creds_json","").strip():
            return jsonify({"leads": [], "error": "Google Sheets not configured — go to Settings."})
        ws   = _get_sheet(s)
...
def api_verify_keys():
    ...
    if key:
        try:
            ...
            res["google_maps"] = "ok" if st in ("OK", "ZERO_RESULTS") else f"API error: {st}"
        except Exception as e:
...
def _run_pipeline():
    s = load_state()
    s.update({"running": True, "last_error": "", "progress": "Starting…",
              "progress_pct": 0, "started_at": datetime.now().isoformat()})
    save_state(s)
    ...
    try:
        ...
        _log(f"Target: {target} leads | Split: {sp.get('no')}% no-site / {sp.get('bad')}% bad / {sp.get('ok')}% ok")
        _log(f"Cities: {len(cities)} | Categories: {len(cats)}")

        # ── PARALLEL SCRAPE ────────────────────────────────────────────
        _set_progress("Scraping Google Maps & JustDial in parallel…", 5)
        combos = [(c, k) for c in random.sample(cities, len(cities))
        ...
        with ThreadPoolExecutor(max_workers=8) as ex:
            ...
            for fut in as_completed(futs):
                ...
                try:
                    ...
                    raw.extend(res)
                    city, cat = futs[fut]
                    _log(f"🔍 {cat[:20]:20} / {city:14} → {len(res)} raw (pool: {len(raw)})")
                    pct = min(40, int((completed_combos / total_combos) * 40))
                    _set_progress(f"Scraped {len(raw)} raw leads…", pct)
                except Exception as e:
                    _log(f"   Scrape error: {e}")

        _log(f"\n📦 Raw pool: {len(raw)} | Enriching top {min(target * 2, len(raw))}…")

        # ── PARALLEL ENRICHMENT ────────────────────────────────────────
        _set_progress(f"Enriching {min(target * 2, len(raw))} leads (email + Instagram)…", 42)
        to_enrich = raw[:target * 2]
        enriched  = []
        done_n    = 0

        def enrich(lead):
            lead["email"]     = _find_email(lead["name"], lead.get("city", ""),
                                            lead.get("website", ""), cfg)
            lead["instagram"] = _find_instagram(lead["name"], lead.get("city", ""),
                                                lead.get("website", ""))
            ...
            for fut in as_completed(futs):
                try:
                    ...
                    if done_n % 10 == 0:
                        ...
                        _set_progress(f"Enriched {done_n} / {len(to_enrich)} leads…", pct)
        ...
        _log(f"\n📊 Final: {len(final)} leads — no-site:{len(b_no)} | bad:{len(b_bad)} | ok:{len(b_ok)}")
        ...
        s["started_at"]     = None
        save_state(s)
        _log(f"\n✅ DONE! {written} leads written. All-time total: {s['total_leads']}")
        _log("=" * 58)
...
def _search_maps(city: str, cat: str, key: str, max_r: int = 8):
    ...
    try:
        r = requests.get(
            "https://maps.googleapis.com/maps/api/place/textsearch/json",
            params={"query": f"{cat} in {city}, India", "key": key, "region": "in"},
            timeout=12
        )
        data = r.json()
...
JD_CAT = {
    ...
    "jewelry store": "jewellery-showrooms", "gym": "gym-fitness-centres",
    "dental clinic": "dentists", "interior designer": "interior-designers",
...
def _scrape_jd(city: str, cat: str, max_r: int = 6):
    ...
    try:
        ...
        cards = (soup.select("li[class*='cntanr']") or
                 soup.select(".resultbox_info") or
                 soup.select("[class*='resultbox']"))
        out = []
...
def _find_email(name: str, city: str, website: str, cfg: dict) -> str:
    ...
    SKIP = {"example", "noreply", "wix", "sentry", "test",
    ...
    # 1. Scrape website
    if website and _cls(website) in ("ok",):
        for pg in [website, website.rstrip("/") + "/contact"]:
            try:
                r = req.get(pg, headers=HDR, timeout=4, allow_redirects=True)
                if r.status_code == 200:
                    from bs4 import BeautifulSoup
                    soup = BeautifulSoup(r.text, "html.parser")
    ...
    try:
        r = req.get(
            ...
            params={"q": f'"{name}" "{city}" email contact site'},
...
def _find_instagram(name: str, city: str, website: str) -> str:
    import requests as req, re
    SKIP = {"p", "explore", "accounts", "stories", "reels",
    ...
    if website:
        try:
            r = req.get(website, headers=HDR, timeout=4)
            from bs4 import BeautifulSoup
            soup = BeautifulSoup(r.text, "html.parser")
...
def _score(lead: dict) -> int:
    ...
    if lead.get("instagram"): s += 15
    try:
        if float(lead.get("rating", 0)) >= 4.0: s += 10
    except Exception:
        pass
    try:
        if int(lead.get("reviews", 0)) >= 50: s += 10
    except Exception:
        pass
    return min(s, 100)
...
def _dedup(leads):
    ...
    for l in leads:
        ...
        if not p:
            no_phone.append(l)
            continue
        if p not in seen or l.get("score", 0) > seen[p].get("score", 0):
            seen[p] = l
    combined = list(seen.values()) + no_phone
...
def _write_leads(ws, leads):
    existing = set(filter(None, ws.col_values(5)[1:]))  # phone col
    today    = datetime.now().strftime("%Y-%m-%d")
    rows     = []
    for l in leads:
        p = l.get("phone", "")
        if p and p in existing:
            continue
        rows.append([
            today,
            l.get("name", ""),
    ...
    while True:
        try:
            ...
            if s.get("automation_on") and not s.get("running"):
                ...
                if now == cfg.get("schedule_time", "09:00"):
                    _log(f"[Scheduler] ⏰ Auto-triggered at {now}")
                    _run_pipeline()
                    time.sleep(62)
        except Exception as e:
...
def index():
    try:
        ...
        return "<h1>Error: frontend/index.html not found</h1><p>Make sure the file exists next to app.py</p>", 404
